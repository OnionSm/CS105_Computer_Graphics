...
function Painter(context, width, height) {
    ...
    this.drawLine = function(p0, p1, rgba) {
        ...
        var sx = (x0 < x1) ? 1 : -1; // Hướng di chuyển của x
        var sy = (y0 < y1) ? 1 : -1; // Hướng di chuyển của y
        var err = dx - dy;
    
        while (true) {
            this.setPixel(x0, y0, rgba); // Vẽ điểm hiện tại
    
            if (x0 === x1 && y0 === y1) break; // Đạt điểm cuối -> dừng vẽ
    
            var e2 = 2 * err;
            if (e2 > -dy) { err -= dy; x0 += sx; } // Di chuyển theo x
            if (e2 < dx) { err += dx; y0 += sy; } // Di chuyển theo y
        }
    };
