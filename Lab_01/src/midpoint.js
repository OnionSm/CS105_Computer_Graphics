...
function Painter(context, width, height) {
    ...
    {
        var t_x0 = p0[0], t_y0 = p0[1]; // Tọa độ tâm
        var t_x1 = p1[0], t_y1 = p1[1]; // Tọa độ điểm thứ 2
        ...
        while (x <= y) 
        {
            this.setPixel(t_x0 + x, t_y0 + y, rgba); // (x, y)
            this.setPixel(t_x0 - x, t_y0 + y, rgba); // (-x, y)
            this.setPixel(t_x0 + x, t_y0 - y, rgba); // (x, -y)
            this.setPixel(t_x0 - x, t_y0 - y, rgba); // (-x, -y)
            
            this.setPixel(t_x0 + y, t_y0 + x, rgba); // (y, x)
            this.setPixel(t_x0 - y, t_y0 + x, rgba); // (-y, x)
            this.setPixel(t_x0 + y, t_y0 - x, rgba); // (y, -x)
            this.setPixel(t_x0 - y, t_y0 - x, rgba); // (-y, -x)
